...
def propogation(op2):
    ...
    if(_int_type >= 0): 
        ...
        if(True):
            ...
            srwl_uti_save_intens_ascii(wp_arI, wfr2.mesh, "CCD_Intensity(50m).dat", 0, ['Photon Energy', 'Horizontal Position', 'Vertical Position', ''], _arUnits=['eV', 'm', 'm', sValUnitName])
        return wp_arI           
...
def main():
    ...
    yResloutionRatio = 1 #xRatio * (srwl_bl.wfr.mesh.xFin - srwl_bl.wfr.mesh.xStart)/(srwl_bl.wfr.mesh.yFin - srwl_bl.wfr.mesh.yStart)
    global wfr2 
